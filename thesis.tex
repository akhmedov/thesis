% \documentclass[type=phd,guide=mon2017dev.karazin]{mon2017dev}[2019/11/16]
\documentclass[type=c,guide=mon2017dev.karazin]{mon2017dev}[2019/11/16]

\usepackage[T2A]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage[english,russian,ukrainian]{babel}

\usepackage[intlimits]{amsmath}
\allowdisplaybreaks
\usepackage{amsthm}
\usepackage{amssymb}

% Таблиці зі стовпчиками, що розтягуються
\usepackage{tabularx}
\usepackage{geometry}
\geometry{hmargin={30mm,15mm},lines=29,vcentering}

% експорт зображень
\usepackage{graphicx}
\graphicspath{{Application/}}

\usepackage{eqparbox} 
\usepackage[x11names]{xcolor}

\usepackage{subfig}
\renewcommand\thesubfigure{\asbuk{subfigure}}

% нумкрация теорем/лемм
\theoremstyle{plain}
\newtheorem{theorem}{Теорема}[chapter]
\newtheorem{lemma}{Лема}[chapter]
\newtheorem{corollary}{Наслідок}[chapter]
\theoremstyle{definition}
\newtheorem{definition}{Означення}[chapter]
\newtheorem{example}{Приклад}[chapter]
\theoremstyle{remark}
\newtheorem{remark}{Зауваження}[chapter]

\include{alias} % Локальние определения

\begin{document}

% Випромінювання нестаціонарних полів та їх розповсюдження в нелінійному просторі
\title(uk){Поле імпульсних антен в лінійному і нелінійному середовищі}
\title(en){The Field of Impuple Radiating Antenna in Linear and Nonlinear Medium}

\author(uk){Ахмедов Ролан Джавадович}
\author(en){Akhmedov Rolan Dhavadovich}

\supervisor(uk){Думін Олександр Миколайович}
{кандидат фізико-математичних наук, доцент}{}

\speciality(uk)
[specialityname=Радіофізика і електроніка, degreefield=фiзико-математичних наук, 
specialityfile=specsci20150406n394.uk.csv]
{01.04.03}

\speciality(en)
[specialityname=Radiophisics and electronics, degreefield=Phisics and mathematics,
specialityfile=specsci20150406n394.en.csv]
{01.04.03}

\udc{537.87}

\institution(uk)[altname=ХНУ імені В. Н. Каразіна]
{Харківський національний університет імені В. Н. Каразіна}
{Харків}
\institution(en)[altname=Karazin KNU]
{V. N. Karazin Kharkiv National University}
{Kharkiv}

\date{2020}

\maketitle

\include{abstract}

\tableofcontents

\include{intro}
\include{abbreviation}
\include{review}
\include{linear}
\include{nonlinear}
\include{neuron}

\include{concl}
\include{bib}

\appendix
\include{trigonometric}
% \include{vector}
\include{bessel}
\include{lommel}
\include{appbib}

\end{document}
